...
;; (defmethod render-field :around ((field input-field) &key)
;;   (with-output-to-string (s)
;;     (let ((*output-stream* s))
;;       (when (field-label field)
;;         (markup (:label :for (field-name field) (field-label field))))
;;       (call-next-method))))
...
(defparameter *form-template-directory* "forms/")
...
(defun render-form (form &key (template *default-form-template*) env)
  (let* ((form-path (merge-pathnames template *form-template-directory*))
         (full-form-path (merge-pathnames form-path *template-directory*))
         (emb:*escape-type* :raw)
